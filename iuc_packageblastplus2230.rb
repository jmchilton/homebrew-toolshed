...
class IucPackageblastplus2230 < Formula
  version "1.0"
  # Recipe auto-generate from repository https://toolshed.g2.bx.psu.edu/iuc/package_blast_plus_2_2_30
  # Tool Shed Readme:
  #    
  ...
  if OS.mac? and Hardware.is_64_bit? and !build.without?("architecture")
    ...
    url "ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/2.2.30/ncbi-blast-2.2.30+-ia32-linux.tar.gz"
    sha1 ""
  ...
  def install
    if OS.mac? and Hardware.is_64_bit? and !build.without?("architecture")
      ...
      environment([{'action'=> 'prepend', 'variable'=> 'PATH', 'value'=> '$KEG_ROOT'},{'action'=> 'set', 'variable'=> 'BLAST_ROOT_DIR', 'value'=> '$KEG_ROOT'}])
    elsif OS.mac? and Hardware.is_32_bit? and !build.without?("architecture")
      prefix.install Dir["bin/*"]
      environment([{'action'=> 'prepend', 'variable'=> 'PATH', 'value'=> '$KEG_ROOT'},{'action'=> 'set', 'variable'=> 'BLAST_ROOT_DIR', 'value'=> '$KEG_ROOT'}])
    elsif OS.linux? and Hardware.is_64_bit? and !build.without?("architecture")
